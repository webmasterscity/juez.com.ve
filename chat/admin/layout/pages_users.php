...
<?php
				$result2 = $db->execute("
					SELECT chat.to 
					FROM chat 
					WHERE chat.from = '" . $db->escape_string(get_var('id')) . "' 
					GROUP BY chat.to
				");
...
<?php
			...
			$result = $db->execute("
				SELECT * 
				FROM chat 
				WHERE (chat.to = '" . $db->escape_string(get_var('w')) . "' 
						AND chat.from = '" . $db->escape_string(get_var('id')) . "') 
					OR (chat.to = '" . $db->escape_string(get_var('id')) . "' 
						AND chat.from = '" . $db->escape_string(get_var('w')) . "') 
				ORDER BY id ASC
			");
					...
					<fieldset class="firstFieldset">
						...
						<dl class="selectionBox">
							...
							<dd>
								...
								<p class="explain">
									Hide or show the chat bar for this user.
								</p>
							</dd>
						</dl>
						<dl class="selectionBox">
...
<?php	
		$result = $db->execute("
			SELECT chat_status.userid userid, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " name
			FROM " . TABLE_PREFIX . DB_USERTABLE . " 
			LEFT JOIN chat_status
				ON " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " = chat_status.userid
			WHERE chat_status.is_mod = 1
			ORDER BY chat_status.userid ASC
		");
...
?>
			<div class="title_bg"> 
				...
				<div class="module_content">
					...
					<div class="subExplain"><i>Users that are set to groups from your main site can have their chat permissions set here.</i></div>
...
?>
			...
			<div class="title_bg"> 
				<div class="title">chat Groups</div> 
				<div class="module_content">
					<div class="subtitle">chat Permissions</div>
					<div class="subExplain"><i>Any guest permissions above will not work. You must control guest permissions via the chat guest group.</i></div>
					<h2 class="subHeading">Groups</h2>
					<ol class="scrollable">
						<li class="listItem">
					...
					<fieldset class="firstFieldset">
						<dl class="selectionBox">
							...
							<dd>
								...
								<p class="explain">
									Checking this will switch all group permissions from 'Disable {Feature}' to 'Enable {Feature}'.<br /><br />WARNING: This will reset all group permissions. If checked, everyone will have chat disabled until permissions are enabled.
								</p>
							</dd>
						</dl>
					</fieldset>
...
<?php
	if ($do == "groupsedit") 
	{
		if (!empty($_REQUEST['gid'])) 
		{
			...
			{
				$group_exists = true;
				$groups_name = "chat Guests";
			}
			
			$group_disable_chat_ex = explode(",", $group_disable_chat);
					...
					<fieldset class="firstFieldset">
						<dl class="selectionBox">
							...
							<dd>
								<ul>
									<li>
										<label for="group_disable_chat">
											<input type="checkbox" id="group_disable_chat" name="group_disable_chat" <?php if(in_array($request_id, $group_disable_chat_ex)) echo 'checked="checked"';  ?> value="1" />
											<?php if ($group_enable_mode == 1) echo 'Enable'; else echo 'Disable'; ?> chat
										</label>
									</li>
								</ul>
								<p class="explain">
									<?php if ($group_enable_mode == 1) echo 'Enable'; else echo 'Disable'; ?>s chat for this group.
								</p>
							</dd>
						</dl>
					</fieldset>
