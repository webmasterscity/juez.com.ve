...
class Imagenes
{
	...
	{	try
		{	$propiedadImagen	= getimagesize($imagen);
			if(!$propiedadImagen)	
			{	throw new Exception(Imagenes::$msjError["ERR_File"]);	}
			...
			{	$this->propiedadesImagen	= $propiedadImagen;
				...
				{	if(!$valor)
					{	throw new Exception(Imagenes::$msjError["ERR_Param"]);	}
				}
			}
			else
			{	throw new Exception(Imagenes::$msjError["ERR_Params"]);	}
			// Verifico que si el modo es 2 existan datos para ello
			if($this->modo==2)
			{	if($this->recorte['filas']<1 || $this->recorte['columnas']<1)
				{	throw new Exception(Imagenes::$msjError["ERR_Modo2"]);	}
			} 
			...
			{	throw new Exception(Imagenes::$msjError["ERR_Exten"]);	}
			
			$this->extencion	= $extencion;
			
			if(!$tamanio)
			{	throw new Exception(Imagenes::$msjError["ERR_Taman"]);	}
			
			// Veo que sea un tipo permitido y que sea efectivamente de ese tipo.
			if(!array_key_exists($extencion,$this->imageCreateFrom))
			{	throw new Exception(Imagenes::$msjError["ERR_ImgNP"]);	}
			...
			if(!img)
			{	throw new Exception(Imagenes::$msjError["ERR_ImgNC"]);	}
	...
	{	if(!$this->punteroImagen)
		{	$this->mensaje	= Imagenes::$msjError["ERR_ImgNS"];
			return false;
		}
	...
	{	// Comprobamos el Origen y Destino de la imagen que existan
		...
		{						
		$this->mensaje	= Imagenes::$msjError["ERR_Ori"];
			return false;
		}
		
		if($this->imagenDestino)
		{	
		$this->mensaje	= Imagenes::$msjError["ERR_Des"];	
			return false;
		}
