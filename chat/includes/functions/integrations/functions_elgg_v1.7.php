<?php
	...
	*/
	...
	function get_friend_list($userid, $time) 
	{
		...
		$sql = ("
			SELECT DISTINCT " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " userid, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " username, chat_status.session_time lastactivity, CONCAT(" . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . ",'^',(SELECT value from " . TABLE_PREFIX . "datalists WHERE name = 'path')) avatar, CONCAT((SELECT username from " . TABLE_PREFIX . "users_entity where guid = friends.guid_two),'^',(SELECT value from " . TABLE_PREFIX . "datalists WHERE name = 'path')) link, chat_status.is_admin, chat_status.status 
