(function (a) {
    ...
    a.chat = function () {
        ...
        function Sa(b) {
            var c = b.substr(19);
			a("#chat_userlist_" + c + " .mobile_alert").hide();
        ...
        function Va(b) {
            ...
            b && a.each(b, function (i, e) {
                ...
                if (buddylistreceived == 1) {
                    ...
                    R = onlineNumber;
                    totalFriendsNumber == 0 && a("#chat_popout_friends").html('<div class="chat_nofriends">' + lang[8] + "</div>");
		...
		function buildChatroomList(b) {
			...
			b && a.each(b, function (i, e) {
				if (i == "chatrooms") {
					a.each(e, function (l, f) {
						if (f.t == 2) {
							alink = '<a href="#password-page" id="chat_chatroom_'+f.id+'" data-rel="dialog" data-transition="slidedown" class="ui-link-inherit chatroom-window-password">';
						} else {
							alink = '<a data-transition="slide" id="chat_chatroom_'+f.id+'" href="#page3" class="ui-link-inherit chatroom-window">';
						}
						if (a("#chat_chatroom_" + f.id + " .mobile_alert").is(":visible"))
							alert_show = ' style="display:block"';
						else
							alert_show = '';
						chatroomlist += '<li data-theme="c" class="ui-btn ui-btn-icon-right ui-li-has-arrow ui-li ui-btn-up-c"><div class="ui-btn-inner ui-li"><div class="ui-btn-text chat_chatroomlist_' + f.t + '">' + alink + '<div class="mobile_avatar_chatroom"><img src="' + c_ac_path + "themes/" + u_theme + '/images/icons/' + f.img + '"></div><div class="chatroom_name_wrapper"><div class="chatroom_name">' + f.n + '</div><div class="chatroom_desc">' + f.d + '</div></div><div class="chatroom_count">'+f.c+lang[35]+'</div><div class="mobile_alert"' + alert_show + '>'+lang[115]+'</div></a></div><span class="ui-icon ui-icon-arrow-r ui-icon-shadow"></span></div></li>';
					});
				}
			});
			chatroomreceived = 1;
		...
		function chatroomListClicked(b) {
			...
			a("#chat_chatroom_" + c + " .mobile_alert").hide();
			a(".back_buttons .ui-btn-text").html(lang[113]);
			Ccr = c;
			Ccr2 = c;
		}
				...
				success: function (o) {
					if (o) {
						...
						if (no_error) {
							...
							$chatroom_chat.html('<div id="chat_chatroom_chat_content"></div>');
							o && a.each(o, function (i, e) {
								if (i == "user_title") {
									a.each(e, function (l, f) {
										if (f.admin == 1) {
								...
								if (i == "chat_users") {
									...
									a.each(e, function (l, f) {
										...
										a("#chatroom-users-list").append('<li data-theme="a" class="ui-btn ui-btn-icon-right ui-li-has-arrow ui-li ui-btn-up-a chat_chatroom_admin_' + f.t + '"><div class="ui-btn-inner ui-li"><div class="ui-btn-text" id="chat_chatroom_user_' + f.id + '"><a data-transition="slide" href="#" class="ui-link-inherit user-window">' + avatar + '<span class="list_name">' + f.n + '</span></a></div><span class="ui-icon ui-icon-arrow-r ui-icon-shadow"></span></div></li>');
										if (f.t == 2) {
											a("#chat_chatroom_title_" + f.id).html(longname + '<br/>' + lang[44]);
										} else if (f.t == 3) {
											a("#chat_chatroom_title_" + f.id).html(longname + '<br/>' + lang[45]);
										} else if (f.t == 4) {
											a("#chat_chatroom_title_" + f.id).html(longname + '<br/>' + lang[212]);
										}
										if (f.id != u_id) {
									...
									a.each(e, function (l, f) {
										if (typeof(blockList[f.userid]) == "undefined") {
											...
											if (f.n == u_name) {
												...
												if (c_disable_avatars != 1)
													sender_avatar = '<div class="chat_sender_avatar"><img src="' + f.a + '" /></div>';
												arrow = 'from';
											}
											var sent_time = new Date(f.t * 1E3);
											if (f.global == 1) {
												d += '<div class="chat_message_wrapper"><div class="chat_chatroom_box_message chat_chatroom_global" id="chat_chatroom_message_' + f.id + '"><div class="chat_chatroom_message_content chat_global_chatroom_message">' + f.m + "</div></div></div>"
											} else {
												d += '<div class="chat_message_wrapper">'+sender_avatar+'<div class="chat_chatroom_box_message' + l + '" id="chat_chatroom_message_' + f.id + '"><img src="'+c_ac_path+'public/mobile/includes/css/images/img-'+arrow+'-arrow.png" class="'+arrow+'-arrow" /><div class="chat_chatroom_message_content"><div class="chat_chatroom_name">' + fromname + title + '</div><span class="chat_chatroom_msg">' + f.m + "</span></div></div></div>"
											}
										}
									});
									a(".chat_room_content #chat_chatroom_chat_content").html(d);
									showChatroomTime();
									setTimeout(function() {
										scrollOnPage3();
									}, 400);
								...
								if (i == "room_info") {
									a.each(e, function (l, f) {										
										if (f.welcome_msg != "") {
											...
											$chatroom_chat.append('<div class="chat_message_wrapper"><div class="chat_chatroom_box_message chat_chatroom_global" id="chat_chatroom_welcome_msg"><div class="chat_chatroom_message_content chat_global_chatroom_message">' + message + "</div></div></div>");
										}
									});
								}
							});
		...
		function setUserOptions(f, global_admin, global_mod) {
			a('#chat_chatroom_user_' + f.id).click(function() {
				...
				a("#msg_button").click(function() {
					a("#chat_userlist_" + f.id + " .mobile_alert").hide();
					a(".back_buttons .ui-btn-text").html(lang[113]);
		...
		function addMessageToChatroom(b, c, d) {
			...
			d = smileyreplace(d);
			if (a("#chat_chatroom_message_" + b).length > 0) {
			} else {
				$chatroom_chat.append('<div class="chat_message_wrapper"><div class="chat_chatroom_box_message chat_self" id="chat_chatroom_message_' + b + '"><img src="'+c_ac_path+'public/mobile/includes/css/images/img-send-arrow.png" class="send-arrow" /><div class="chat_chatroom_message_content"><div class="chat_chatroom_name">' + c + title + '</div><span class="chat_chatroom_msg">' + d + '</span></div></div></div>');
				scrollOnPage3();
			}
			showChatroomTime();
			a("#textinput2").focus();
			...
			for (i = 0; i < Smiley.length; i++) {
				...
				if (check_emoticon != -1) {
					mess = mess.replace(
						...
						'<img class="chat_smiley" height="16" width="16" src="' + c_ac_path + "themes/" + u_theme + '/images/smilies/' + Smiley[i][0] + '.png" alt="" />'
					);
				}
			}
			return mess;
        ...
        function sa(b, c, d) {
            if (uc_name[b] == null || uc_name[b] == "") setTimeout(function () {
                ...
                Oa(b);
                if (c == 0) {
                    a("#chat_userlist_" + b + " .mobile_alert").hide();
                } else {
                    a("#chat_userlist_" + b + " .mobile_alert").show();
                }
                y[b] = c;
            }
        }
        ...
        function X(b, c, d, i, e, l, f) {
            ...
            if (uc_name[b] == null || uc_name[b] == "") setTimeout(function () {
                ...
                d != 1 && u_sounds == 1 && Ua();
                separator = ":&nbsp;&nbsp;";
                if (a("#chat_message_" + e).length > 0) a("#chat_message_" + e + " .chat_chatboxmessagecontent").html(c);
                else {
                    sentdata = "";
                    ...
                    if (f - B > 180) {
                        a("#chat_user_" + b).append('<div class="chat_ts_wrapper">' + sentdata + '</div><div class="chat_message_wrapper"><div class="chat_chatboxmessage' + h + '" id="chat_message_' + e + '"><img src="'+c_ac_path+'public/mobile/includes/css/images/img-'+arrow+'-arrow.png" class="'+arrow+'-arrow" /><div class="chat_chatboxmessagecontent" style="margin-left: 0">' + c + "</div></div></div>");
                        B = f;
                        N = full_name;
                    } else a("#chat_user_" + b).append('<div class="chat_message_wrapper"><div class="chat_chatboxmessage' + h + '" id="chat_message_' + e + '"><img src="'+c_ac_path+'public/mobile/includes/css/images/img-'+arrow+'-arrow.png" class="'+arrow+'-arrow" /><div class="chat_chatboxmessagecontent" style="margin-left: 0">' + c + "</div></div></div>");
					scrollOnPage2();
                }
                j != b && i != 1 && sa(b, 1, 1)
            }
        ...
        function ya(b) {
            ...
            else j != b && a("#chat_popout_user_" + b).click()
        }
        function Ua() {
        ...
        function ha(b) {
			...
			if (c_us_time != 1) {
				return e != Na ? '<span class="chat_ts">' + l[b] + " " + e + f + ", " + g + ":" + i + "</span>" : '<span class="chat_ts">' + g + ":" + i + "</span>"
			} else {
				return e != Na ? '<span class="chat_ts">' + l[b] + " " + e + f + ", " + d + ":" + i + c + "</span>" : '<span class="chat_ts">' + d + ":" + i + c + "</span>"
			}
        }
        function La(b) {
            a.ajax({
                ...
                success: function (c) {
                    if (c) {
                        a("#chat_user_" + b + "_popup .chat_popout_convo").html("");
                        B = null;
                        var d = "",
							sender_avatar = "",
							arrow = "",
                        ...
                        a.each(c, function (e, l) {
                            e == "messages" && a.each(l, function (f, h) {
                                ...
                                if (h.self == 1) {
                                    ...
                                    _aa5 = "</a>"
									if (c_disable_avatars != 1)
										sender_avatar = '<div class="chat_sender_avatar"><img src="' + uc_avatar[b] + '" /></div>';
									arrow = 'from';
                                }
								var full_name = fromid;
								...
								if (a("#chat_message_" + h.id).length > 0) {
									a("#chat_message_" + h.id + " .chat_chatboxmessagecontent").html(h.message);
								} else {
									if (h.sent - B > 180 || B == null) {
										d += '<div class="chat_ts_wrapper">' + ha(o) + '</div><div class="chat_message_wrapper">'+sender_avatar+'<div class="chat_chatboxmessage' + f + '" id="chat_message_' + h.id + '"><img src="'+c_ac_path+'public/mobile/includes/css/images/img-'+arrow+'-arrow.png" class="'+arrow+'-arrow" /><div class="chat_chatboxmessagecontent" style="margin-left:0">' + h.message + "</div></div></div>";
										B = h.sent;
										N = full_name;
									} else d += '<div class="chat_message_wrapper">'+sender_avatar+'<div class="chat_chatboxmessage' + f + '" id="chat_message_' + h.id + '"><img src="'+c_ac_path+'public/mobile/includes/css/images/img-'+arrow+'-arrow.png" class="'+arrow+'-arrow" /><div class="chat_chatboxmessagecontent" style="margin-left:0">' + h.message + "</div></div></div>"
								}
                            })
                        });
                        if (a("#chat_user_" + b).length > 0) oa("chat_user_" + b, d + document.getElementById("chat_user_" + b).innerHTML);
                        else document.getElementById("chat_user_" + b).innerHTML = d;
						setTimeout(function() {
							scrollOnPage2();
						}, 400);
                    }
        ...
        function qa(b, c, d, e, l, f) {
			...
			a(".chat_user_content").attr("id", "chat_user_" + b);
			scrollOnPage2();
			if (a("#chat_userlist_" + b).length > 0) {
			} else {
				...
				a("#chat_userlist_" + b).click(function () {
					var tba = 0;
                    if (a("#chat_userlist_" + b + " .mobile_alert").is(":visible")) {
                        tba = 1;
                        a("#chat_userlist_" + b + " .mobile_alert").hide();
                        G[b] = 0;
                        y[b] = 0;
					...
					if (j != "") {
						a("#chat_popout_user_" + j).removeClass("chat_popout_focused");
						a("#chat_popout_user_" + j + "_convo").removeClass("chat_popout_convo_focused");
						j = ""
					}
					La(b);
					a.post(c_ac_path + "includes/json/send/send_settings.php", {
					...
					a(this).addClass("chat_popout_focused");
					a("#chat_popout_user_" + b + "_convo").addClass("chat_popout_convo_focused");
					j = b;
                    scrollOnPage2();
				});
				f != 1 && a("#chat_popout_user_" + b).click();
				y[b] = 0;
				G[b] = 0;
        }
		...
		function receiveMessage(id, from, message, sent, self, old) {
			if (j != from) {
				a("#chat_userlist_" + from + " .mobile_alert").show();
			...
			if (j == from && uc_name[from] != "" && uc_name[from] != null) {
				...
				if (self == 1) {
					...
					_aa5 = "</a>";
					if (c_disable_avatars != 1)
						sender_avatar = '<div class="chat_sender_avatar"><img src="' + uc_avatar[from] + '" /></div>';
					arrow = 'from';
				}
				var full_name = fromid;
				...
				if (a("#chat_message_" + id).length > 0) {
					...
					if (sent - B > 180 || B == null) {
						c += '<div class="chat_ts_wrapper">' + ha(o) + '</div><div class="chat_message_wrapper">'+sender_avatar+'<div class="chat_chatboxmessage' + f + '" id="chat_message_' + id + '"><img src="'+c_ac_path+'public/mobile/includes/css/images/img-'+arrow+'-arrow.png" class="'+arrow+'-arrow" /><div class="chat_chatboxmessagecontent" style="margin-left:0">' + message + "</div></div></div>";
						B = sent;
						N = full_name;
					} else c += '<div class="chat_message_wrapper">'+sender_avatar+'<div class="chat_chatboxmessage' + f + '" id="chat_message_' + id + '"><img src="'+c_ac_path+'public/mobile/includes/css/images/img-'+arrow+'-arrow.png" class="'+arrow+'-arrow" /><div class="chat_chatboxmessagecontent" style="margin-left:0">' + message + "</div></div></div>"
				}
			} else {
				message = stripslashes(message);
		...
		function receiveTyping(id) {
			a("#chat_popout_user_"+id+" .chat_closebox_bottom_status").addClass("chat_typing")
		}
		function receiveNotTyping(id) {
			a("#chat_popout_user_"+id+" .chat_closebox_bottom_status").removeClass("chat_typing")
		}
		...
		function chatroomDiv(id, image, name, time, message, global, mod, admin, chatroomid) {
			...
			if (u_name == name) {
				...
				if (c_disable_avatars != 1)
					sender_avatar = '<div class="chat_sender_avatar"><img src="' + image + '" /></div>';
				arrow = "from";
			}
			if (a("#chat_chatroom_message_" + id).length > 0) {
				...
				if (global == 1) {
					a("<div/>").addClass("chat_message_wrapper").html('<div class="chat_chatroom_box_message chat_chatroom_global" id="chat_chatroom_message_' + id + '"><div class="chat_chatroom_message_content chat_global_chatroom_message">' + message + "</div></div>").appendTo($chatroom_chat);
				} else {
					a("<div/>").addClass("chat_message_wrapper").html(sender_avatar+'<div class="chat_chatroom_box_message' + self + '" id="chat_chatroom_message_' + id + '"><img src="'+c_ac_path+'public/mobile/includes/css/images/img-' + arrow + '-arrow.png" class="' + arrow + '-arrow" /><div class="chat_chatroom_message_content"><div class="chat_chatroom_name">' + name + title + '</div><span class="chat_chatroom_msg">' + message + "</span></div></div>").appendTo($chatroom_chat);
				}
				if (Ccr != chatroomid && global != 1) {
					a("#chat_chatroom_" + chatroomid + " .mobile_alert").show();
					if (a.mobile.activePage.attr("id") != "page1"){a(".back_buttons .ui-btn-text").html(lang[113] + ' (1)');}
				}
			}
		...
		function renderHTMLString(string) {
			...
			var new_render = a("#chat_render").html()
			render.remove();
			return new_render;
		}
        var bounce = 0,
        ...
        arguments.callee.runchat = function () {
            Xa()
        };
    }
})(jqac);
