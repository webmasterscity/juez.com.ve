	...
	*/
		...
		if (empty($error)) 
		{
			...
			if ($result AND $db->count_select() > 0) 
			{
				...
				if (empty($error)) 
				{
					...
					{
						$error = "Your theme is already up-to-date or you haven't uploaded the new theme files to your server. Download the new files from the chat store and overwrite all the files on your server.";
					}
	...
	{	
		$result = $db->execute("
			SELECT * 
			FROM chat_themes 
			WHERE chat_themes.folder = '" . get_var('theme_folder') . "' 
				OR chat_themes.name = '" . get_var('theme_name') . "'
		");
	...
	if (var_check('theme_default'))
	{
		...
		if (empty($error)) 
		{
			...
			$result = $db->execute("
				UPDATE chat_themes 
				SET chat_themes.active = '1', 
					chat_themes.default = '1' 
				WHERE id = '" . $theme_id . "'
			");
	...
	if (var_check('theme_edit_submit')) 
	{
		$result = $db->execute("
			...
			WHERE (chat_themes.folder = '" . get_var('theme_folder') . "' 
					OR chat_themes.name = '" . get_var('theme_name') . "') 
				AND chat_themes.id != '" . get_var('theme_id') . "'
		");
		...
		if (empty($error)) 
		{
			...
			$result = $db->execute("
				UPDATE chat_themes 
				SET chat_themes.folder = '" . get_var('theme_folder') . "', 
					chat_themes.name = '" . get_var('theme_name') . "', 
					chat_themes.active = '" . get_var('theme_active') . "', 
					chat_themes.default = '" . get_var('theme_default') . "' 
	...
	if (var_check('smiley_submit')) 
	{
		for ($i = 1; $i < get_var('smiley_count'); $i++) 
		{
			...
			$result = $db->execute("
				UPDATE chat_smilies 
				SET chat_smilies.name = '" . $db->escape_string($name) . "', 
					chat_smilies.code = '" . stripslashes($db->escape_string($pattern)) . "' 
				WHERE chat_smilies.id = '" . $db->escape_string($smileyid) . "'
			");
		}
	...
	if (var_check('activate')) 
	{
		...
		$result = $db->execute("
			SELECT chat_themes.default 
			FROM chat_themes 
			WHERE id = '" . get_var('id') . "' 
				AND chat_themes.default = '1'
		");
