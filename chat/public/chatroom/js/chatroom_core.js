(function (a) {
	...
	a.chat = function () {
		...
		function chatroomUserOptions(data, is_admin) {
			...
			a("#chat_chatroom_block_user_" + data.id).click(function () {
				a.post(c_ac_path + "includes/json/send/send_settings.php", {
					...
					if (json_data != "-1") {
						...
						displayMessage("chat_chatroom_message_flyout", lang[103], "error");
					}
				});
				toggleChatroomUserInfo(data.id);
			});
			if (c_enable_moderation != 1) a("#chat_chatroom_report_user_" + data.id).hide();
			...
			a("#chat_chatroom_visit_profile_" + data.id).click(function () {
				window.open(data.l);
			});
			...
			a("#chat_chatroom_user_" + data.id).click(function () {
				...
				crou = data.id;
				a(this).toggleClass("chat_chatroom_clicked");
				a("#chat_chatroom_users_flyout_" + data.id).toggleClass("chat_chatroom_create_flyout_display");
			});
		}
			...
			if (key.keyCode == 13 && key.shiftKey == 0) {
				...
				if (c_send_room_msg == 1 && i != "") {
					...
					i != "" && a.ajax({
						...
						beforeSend: function () {
							a(".chat_popout_convo_input").addClass("chat_message_sending_popout");
						},
						error: function () {
							a(".chat_popout_convo_input").removeClass("chat_message_sending_popout");
						...
						success: function (e) {
							...
							a(".chat_popout_convo").scrollTop(6E4);
						}
						}
					});
				}
		...
		function modDeleteControls() {
			if (chatroom_mod == 1 || chatroom_admin == 1) {
				...
				a(".chat_chatroom_delete").unbind("mouseenter").unbind("mouseleave").unbind("click");
				a(".chat_chatroom_delete").mouseenter(function () {
					showTooltip(a(this), lang[160], 0, 3, 21);
					a(this).addClass("chat_chatroom_delete_hover")
				});
				a(".chat_chatroom_delete").mouseleave(function () {
					hideTooltip();
		...
		function addMessageToChatroom(b, c, d) {
			...
			if (a("#chat_chatroom_message_" + b).length > 0) {
				...
				a(".chat_popout_convo").append('<div class="chat_chatroom_box_message chat_clearfix chat_self' + important + '" id="chat_chatroom_message_' + b + '"><img class="chat_chatroom_message_avatar chat_no_names" src="'+u_avatar+'" alt="' + c + title + '" /><div class="chat_chatroom_message_name">' + c + title + ':</div><div class="chat_chatroom_message_content"><div class="chat_chatroom_delete" data-id="' +  b + '"> </div>' + formatTimestamp(new Date(Math.floor((new Date).getTime() / 1E3) * 1E3)) + '<span class="chat_chatroom_msg">' + d + "</span></div></div>");
        ...
        function receiveChatroom(c) {
            ...
            a.ajax({
				...
				success: function (b) {
					if (b) {
						var no_error = true;
						a(".chat_popout_chatroom_user_flyouts").html('');
							...
							b && a.each(b, function (i, e) {
								...
								if (i == "chat_users") {
									...
									a.each(e, function (l, f) {
										...
										a("<div/>").attr("id", "chat_chatroom_user_" + f.id).css("position", "relative").mouseover(function () {
										...
										a("<div/>").attr("id", "chat_chatroom_users_flyout_" + f.id).css("right","auto").css("left", "15px").css("top", "0px").addClass("chat_more_wrapper_chatroom").html('<div class="chat_chatroom_users_flyout"><div class="chat_chatroom_flyout_avatar"><img src="'+f.a+'" alt="" /></div><div class="chat_chatroom_flyout_info"><div class="chat_chatroom_title_padding"><div id="chat_chatroom_title_' + f.id + '" class="chat_chatroom_flyout_text"><a target="_blank" href="'+f.l+'">' + longname + '</a><br/>' + lang[43] + '</div></div><hr class="chat_options_divider"/><div class="chat_chatroom_options_padding"><div id="chat_chatroom_block_user_' + f.id + '" class="chat_chatroom_flyout_text">' + lang[84] + '</div></div><div class="chat_chatroom_options_padding"><div id="chat_chatroom_report_user_' + f.id + '" class="chat_chatroom_flyout_text">' + lang[167] + '</div></div>' + admin_markup + '</div><div class="chat_clearfix"></div><i class="chat_more_tip_chatroom"></i></div>').appendTo(a(".chat_popout_chatroom_user_flyouts"));
										if (f.t == 2) {
											a("#chat_chatroom_title_" + f.id).html('<a target="_blank" href="'+f.l+'">' + longname + '</a><br/>' + lang[44])
										} else if (f.t == 3) {
											a("#chat_chatroom_title_" + f.id).html('<a target="_blank" href="'+f.l+'">' + longname + '</a><br/>' + lang[45])
										} else if (f.t == 4) {
											a("#chat_chatroom_title_" + f.id).html('<a target="_blank" href="'+f.l+'">' + longname + '</a><br/>' + lang[212])
										}
		...
		function chatroomUploadProcessing() {
			...
			a("#chat_upload_button").mouseenter(function () {
				a(this).addClass("chat_upload_button_hover")
			});
			a("#chat_upload_button").mouseleave(function () {
				...
				success: function (o) {
					if (o) {
						...
						if (no_error) {
							...
							a(".chat_popout_convo_input").keydown(function (h) {
								return chatroomKeydown(h, a(this))
							});
							var smiley_exist = [];
							a(".chat_smiley_box").html('');
							for (var i = 0; i < Smiley.length; i++) {
								if (a.inArray(Smiley[i][0], smiley_exist) > -1) {
								} else {
									a(".chat_smiley_box").append('<div class="chat_smiley_wrapper" data-id="'+i+'"><img class="chat_smiley" src="'+c_ac_path+'themes/'+u_theme+'/images/smilies/'+Smiley[i][0]+'.png" alt="" /></div>');
									smiley_exist.push(Smiley[i][0]);
								}
							}
							a(".chat_smiley_button").mouseenter(function () {
								a(this).addClass("chat_smiley_button_hover")
							});
							a(".chat_smiley_button").mouseleave(function () {
								a(this).removeClass("chat_smiley_button_hover");
							});
							a(".chat_smiley_wrapper").click(function () {
								var smiley_code = a(this).attr("data-id");
								var existing_text = a(".chat_popout_convo_input").val();
								a(".chat_popout_convo_input").focus().val('').val(existing_text + Smiley[smiley_code][1]);
							});
							a(".chat_smiley_button").click(function () {
							...
							o && a.each(o, function (i, e) {
								...
								if (i == "chat_history") {
									...
									a(".chat_popout_convo #chat_chatroom_chat_content").html(d);
									showChatroomTime();
								}
								if (i == "room_info") {
									a.each(e, function (l, f) {										
										if (f.welcome_msg != "") {
											...
											a(".chat_popout_convo").append('<div class="chat_chatroom_box_message chat_clearfix" id="chat_chatroom_welcome_msg"><div class="chat_chatroom_message_content chat_global_chatroom_message">' + message + "</div></div>");
										}
									});
								}
							});
        ...
        function smileyreplace(mess) {
            for (i = 0; i < Smiley.length; i++) {
                ...
                if (check_emoticon != -1) {
                    mess = mess.replace(Smiley[i][1], '<img class="chat_smiley" height="16" width="16" src="' + c_ac_path + "themes/" + u_theme + '/images/smilies/' + Smiley[i][0] + '.png" alt="" />');
                }
            }
            return mess;
        }
		...
		function M() {
			a(".chat_popout_convo").css("height", a(window).height() - a(".chat_popout_input_container").height() - 10);
		}
		...
		function chatroomDiv(id, image, name, time, message, global, mod, admin, userid) {
			var container = a(".chat_popout_convo")[0].scrollHeight - a(".chat_popout_convo").scrollTop() - 10;
			var container2 = a(".chat_popout_convo").outerHeight();
			...
			if (a("#chat_chatroom_message_" + id).length > 0) {
				...
				if (global == 1) {
					a("<div/>").attr("id", "chat_chatroom_message_" + id).addClass("chat_chatroom_box_message").addClass("chat_clearfix").html('<div class="chat_chatroom_message_content chat_global_chatroom_message">' + formatTimestamp(time) + message + "</div>").appendTo(a(".chat_popout_convo"));
					receiveChatroom(Ccr);
				} else {
					a("<div/>").attr("id", "chat_chatroom_message_" + id).addClass(important).addClass(l).addClass("chat_chatroom_box_message").addClass("chat_clearfix").html('<img class="chat_chatroom_message_avatar chat_no_names" src="'+image+'" alt="' + name + title + '" /><div class="chat_chatroom_message_name">' + name + title + ':</div><div class="chat_chatroom_message_content"><div class="chat_chatroom_delete" data-id="' +  id + '"> </div>' + formatTimestamp(time) + '<span class="chat_chatroom_msg">' + message + "</span></div>").appendTo(a(".chat_popout_convo"));
				}
			}
			if (c_disable_avatars == 1 || u_no_avatars == 1) {
				a(".chat_chatroom_avatar").addClass("chat_hide_avatars");
			...
			if ("chatroommessage" in data) {
				if (typeof(blockList[data.chatroommessage.userid]) == "undefined")
				{
					...
					if (data.chatroommessage.userid != u_id) {
						u_chatroom_sound == 1 && !a(".chat_popout_convo_input").is(":focus") && playNewMessageSound();
					}
				}
			}
		}
		...
		function Xa() {
			if (u_id != "" && c_chatrooms == 1) {
				...
				a("<div/>").attr("id", "chat_popout_user_" + ac_chatroom_id + "_convo").addClass("chat_popout_convo_wrapper").html('<div class="chat_popout_chatroom_user_flyouts"></div><div id="chat_popout_text_' + ac_chatroom_id + '" class="chat_popout_convo" style="padding-left:5px;"><div id="chat_chatroom_chat_content"></div></div><div class="chat_popout_input_container"><div class="chat_smiley_button"><div class="chat_more_wrapper chat_smiley_popout"><div class="chat_more_popout"><div class="chat_smiley_box"></div><i class="chat_more_tip"></i></div></div></div><div id="chat_upload_button"><div id="chat_chatroom_uploader"> </div></div><div class="chat_popout_input_wrapper"><textarea maxlength="' + c_max_chatroom_msg + '" class="chat_popout_convo_input"></textarea></div></div>').appendTo(a("#chat_popout_chat")).show();
				a(".chat_popout_input_container").click(function() {
					a(".chat_popout_convo_input").focus();
				});
				M();
