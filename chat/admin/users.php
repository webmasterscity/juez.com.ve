	...
	*/
	...
	if (var_check('ban_ip_submit')) 
	{
		...
		foreach ($ip_addys as $addy) 
		{
			...
			{
				$result = $db->execute("
					INSERT INTO chat_banlist (ban_ip, banned_by, banned_time) 
	...
	if (var_check('ban_username_submit')) 
	{
		...
		foreach ($usernames as $username) 
		{
			...
			{
				$result = $db->execute("
					INSERT INTO chat_banlist (ban_userid, banned_by, banned_time) 
	...
	if (var_check('user_submit')) 
	{
		...
		else
		{
			...
			$db->execute("
				INSERT INTO chat_status (userid, chatroom_window, chatroom_stay, hash_id, session_time, ip_address) 
	...
	if (var_check('group_submit')) 
	{
		...
		if (get_var('group_disable_chat') == 1)
		{
			if (!in_array($group_id, $group_disable_chat_ex))
			{
				$group_disable_chat_ex[] = $group_id;
			}
		}
		else
		{
			if (in_array($group_id, $group_disable_chat_ex))
			{
				$group_disable_chat_ex = array_delete($group_disable_chat_ex, $group_id);
			}
		}
		...
		$group_disable_chat_new = ltrim(implode(",", $group_disable_chat_ex), ',');
		...
		$result = $db->execute("
			...
			SET config_value = CASE 
				WHEN config_name = 'group_disable_chat' THEN '" . $db->escape_string($group_disable_chat_new) . "'
			...
			END WHERE config_name IN ('group_disable_chat', 'group_disable_video', 'group_disable_apps', 'group_disable_rooms', 'group_disable_uploads', 'group_disable_sending_private', 'group_disable_sending_rooms')
		");
		
		if ($result) 
		{
			$group_disable_chat = $group_disable_chat_new;
	...
	if (var_check('group_settings_submit')) 
	{
		...
		$result = $db->execute("
			UPDATE chat_config 
			SET config_value = CASE 
				WHEN config_name = 'group_disable_chat' THEN ''
