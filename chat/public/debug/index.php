<?php
	...
	*/
	...
	{
		$test_userid = 'The User ID is not set. chat is acting as if no one is logged in which may or may not show the bar depending on your settings. <a href="javascript:;" class="vtip" title="&lt;b&gt;What this means&lt;/b&gt;<br />Depending on your chat admin settings, this may mean several things.  If guests are allowed to chat, the full bar will be displayed. If the option to show a message was chosen, then guests will see a message to login.  If neither of those is true, chat will not be displayed.<br /><br />&lt;b&gt;How to fix this&lt;/b&gt;<br />You can fix the User ID if there is a problem in the /chat/includes/integration.php file. The get_user_id() function must return the logged in user\'s ID.">More Information &rarr;</a>';
	...
	{
		$test_buddylist = 'The buddy list has an error in the MySQL. Here is the error that MySQL is returning:' . $db->display_errors() . '<a href="javascript:;" class="vtip" title="&lt;b&gt;What this means&lt;/b&gt;<br />The buddy list cannot be loaded properly. You are most likely seeing an error when clicking on the chat tab.<br /><br />&lt;b&gt;How to fix this&lt;/b&gt;<br />You can fix the buddy list if there is a problem in the /chat/includes/integration.php file. The get_friend_list() function is used for sites with a friends list. The get_online_list() function is used for sites that want to display all online users.">More Information &rarr;</a>';
	...
	{
		$test_banned = 'Your user ID or IP address is currently banned. You can change this in the chat admin panel.';
	...
	{
		$test_browser = 'You are currently using IE version 5 or 6. chat does not work with this browser.';
