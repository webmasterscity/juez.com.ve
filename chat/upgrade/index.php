<?php
	...
	*/
	...
	{
		$val = $db->execute("SELECT 1 FROM `chat_reports`");
		
		if ($val !== FALSE)
		{
			$result = $db->execute("
				INSERT IGNORE INTO chat_config (config_name, config_value, is_dynamic)
				VALUES ('window_top_padding', '70', '0'), ('video_chat_selection', '1', '0'), ('video_chat_width', '900', '0'), ('video_chat_height', '600', '0'), ('tokbox_api', '', '0'), ('tokbox_secret', '', '0'), ('chatroom_default_names', '0', '0'), ('group_disable_chat', '', '0'), ('group_disable_video', '', '0'), ('group_disable_apps', '', '0'), ('group_disable_rooms', '', '0'), ('group_disable_uploads', '', '0'), ('group_disable_sending_private', '', '0'), ('group_disable_sending_rooms', '', '0'), ('group_enable_mode', '0', '0'), ('online_list_on', '1', '0')
			");
			
			$result = $db->execute("
				ALTER TABLE `chat_chatroom_rooms`
		...
		else
		{
			$result = $db->execute("
				INSERT IGNORE INTO chat_config (config_name, config_value, is_dynamic)
				VALUES ('max_upload_size', '5', '0'), ('enable_moderation', '0', '0'), ('chatroom_transfer_on', '0', '0'), ('chatroom_message_length', '150', '0'), ('push_ssl', '0', '0'), ('window_top_padding', '70', '0'), ('video_chat_selection', '1', '0'), ('video_chat_width', '900', '0'), ('video_chat_height', '600', '0'), ('tokbox_api', '', '0'), ('tokbox_secret', '', '0'), ('chatroom_default_names', '0', '0'), ('group_disable_chat', '', '0'), ('group_disable_video', '', '0'), ('group_disable_apps', '', '0'), ('group_disable_rooms', '', '0'), ('group_disable_uploads', '', '0'), ('group_disable_sending_private', '', '0'), ('group_disable_sending_rooms', '', '0'), ('group_enable_mode', '0', '0'), ('online_list_on', '1', '0')
			");
			
			$result = $db->execute("
				TRUNCATE chat_smilies
			");
			
			$result = $db->execute("
				ALTER TABLE `chat_smilies`
			...
			$result = $db->execute("
				INSERT IGNORE INTO chat_smilies (id, name, code)
			...
			$result = $db->execute("
				ALTER TABLE `chat_chatroom_messages`
				ADD `action` tinyint(1) unsigned default '0' AFTER `sent`
			");
			
			$result = $db->execute("
				ALTER TABLE `chat_chatroom_banlist`
			...
			$result = $db->execute("
				ALTER TABLE `chat_chatroom_users` DROP PRIMARY KEY
			");
			
			$result = $db->execute("
				ALTER TABLE `chat_chatroom_users`
...
<head profile="http://gmpg.org/xfn/11"> 
	...
	<title>chat | Upgrade Installation</title> 
...
<body> 
	...
	<div style="font-size: 14px; font-family: arial, verdana; margin: 0 auto; width: 700px; margin-top: 50px;">
		...
		<span style="font-weight: bold; font-size: 18px;">chat Upgrade Process</span>
...
<?php
	if ($ran) {
?>
		Your chat installation has been upgraded.  It is safe to delete the upgrade folder.
<?php
	} else {
?>
		Running this upgrade process will make the necessary upgrades to your chat database structure.  This is not requried on a fresh install of chat.  It is safe to delete the upgrade folder after this process is run or on a fresh install of chat.
