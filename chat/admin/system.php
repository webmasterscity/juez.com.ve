<?php
	...
	*/
		...
		if (!empty($_SESSION['clean_cr_rooms']) AND empty($_SESSION['clean_cr_messages']) AND empty($_SESSION['clean_inactive_users']) AND empty($_SESSION['clean_inactive_guests']) AND empty($_SESSION['clean_private_messages']))
		{
			...
			if ($row = $db->fetch_array($result))
			{
				...
				if ($no_rows < 5000) 
				{
					...
					{
						$db->execute("
							DELETE FROM chat_chatroom_rooms
							WHERE session_time + (3600 * 24 * 7)  < " . time() . "
								AND length != '0'
						");
						$_SESSION['counter'] = -1;
		...
		if (!empty($_SESSION['clean_cr_users']) AND empty($_SESSION['clean_cr_rooms']) AND empty($_SESSION['clean_cr_messages']) AND empty($_SESSION['clean_inactive_users']) AND empty($_SESSION['clean_inactive_guests']) AND empty($_SESSION['clean_private_messages']))
		{
			...
			if ($row = $db->fetch_array($result))
			{
				...
				if ($no_rows < 5000) 
				{
					...
					{
						$db->execute("
							DELETE FROM chat_chatroom_users
							WHERE session_time + (3600 * 24 * 14)  < " . time() . "
						");
						$_SESSION['counter'] = -1;
	...
	{
		$result = $db->execute("
			INSERT IGNORE INTO chat_config (
				config_name, 
				config_value, 
				is_dynamic
			) 
	...
	if ($do == "step3") 
	{
		...
		//$fp = @fpen("http://www.chat.com/validate.php?u=" . get_var('validate_username') . "&p=" . get_var('validate_password'), "r");
		...
		{
			echo '<script type="text/javascript">document.getElementById(\'download\').innerHTML = "We could not make a connection to chat.com.  Either the site is down or your fopen does not allow remote URLs.";</script>';
			echo '<script type="text/javascript">document.getElementById(\'continue\').innerHTML = "<dl class=\'selectionBox submitBox\'><dt></dt><dd><div class=\'floatr\' style=\'float: right;\'><a class=\'fwdbutton\' onclick=\'document.forms[0].submit(); return false\'><span>Back to Login Details</span></a><input type=\'hidden\' name=\'auto-step-2\' value=\'1\' /></div></dd></dl>";</script>';
			flush_headers();
			exit;
		}
		...
		if (empty($key)) 
		{
			echo '<script type="text/javascript">document.getElementById(\'download\').innerHTML = "There was a problem with your key, please try again or contact chat support.";</script>';
			echo '<script type="text/javascript">document.getElementById(\'continue\').innerHTML = "<dl class=\'selectionBox submitBox\'><dt></dt><dd><div class=\'floatr\' style=\'float: right;\'><a class=\'fwdbutton\' onclick=\'document.forms[0].submit(); return false\'><span>Back to Login Details</span></a><input type=\'hidden\' name=\'auto-step-2\' value=\'1\' /></div></dd></dl>";</script>';
			flush_headers();
			exit;
		}
		...
		// Start downloading chat if it doesn't already exist in the cache folder
		if (file_exists(dirname(dirname(__FILE__)) . DIRECTORY_SEPARATOR . AC_FOLDER_CACHE . DIRECTORY_SEPARATOR . "chat_v" . get_var('url') . ".zip")) 
		{
			echo '<script type="text/javascript">document.getElementById(\'download\').innerHTML = "chat already downloaded...";</script>';
			flush_headers();
		} 
		else 
		{
			...
			$remoteFile = "http://www.chat.com/update-download.php?file=ac&id=" . $key;
			$dir = dirname(dirname(__FILE__)) . DIRECTORY_SEPARATOR . AC_FOLDER_CACHE . DIRECTORY_SEPARATOR;

			$file = @fopen($remoteFile, "r");

			echo '<script type="text/javascript">document.getElementById(\'download\').innerHTML = "Downloading chat...";</script>';
			flush_headers();

			if (!$file) 
			{
				echo '<script type="text/javascript">document.getElementById(\'download\').innerHTML = "Unable to open remote file.  Please download the file manually from the <a href=\'http://www.chat.com\' target=\'_blank\'>chat.com</a> member\'s area and place it in the cache folder.  Do not rename the file.";</script>';
				flush_headers();
				exit;
			}
			...
			file_put_contents(dirname(dirname(__FILE__)) . DIRECTORY_SEPARATOR . AC_FOLDER_CACHE . DIRECTORY_SEPARATOR . "chat_v" . get_var('url') . ".zip",  $line);
			fclose($file);
		}
		...
		else 
		{
			...
			$remoteFile = "http://www.chat.com/update-download-mysql.php?file=ac&id=" . $key;
			$dir = dirname(dirname(__FILE__)) . DIRECTORY_SEPARATOR . AC_FOLDER_CACHE . DIRECTORY_SEPARATOR;
			...
			if (!$file) 
			{
				echo '<script type="text/javascript">document.getElementById(\'download2\').innerHTML = "Unable to open remote file.  Please download the file manually from the <a href=\'http://www.chat.com\' target=\'_blank\'>chat.com</a> member\'s area and place it in the cache folder.  Do not rename the file.";</script>';
				flush_headers();
				exit;
			}
		...
		// Put chat into maintenance mode for installation
		echo '<script type="text/javascript">document.getElementById(\'maintenance\').innerHTML = "Putting chat in maintenance mode...";</script>';
		flush_headers();
		...
		// Unzip the chat zip file and overwrite all files
		echo '<script type="text/javascript">document.getElementById(\'unpacking\').innerHTML = "Unpacking chat...";</script>';
		flush_headers();
		
		$zip = new ZipArchive;
		if ($zip->open(dirname(dirname(__FILE__)) . DIRECTORY_SEPARATOR . AC_FOLDER_CACHE . DIRECTORY_SEPARATOR . "chat_v" . get_var('url') . ".zip") === TRUE) 
		{
		...
		{
			echo '<script type="text/javascript">document.getElementById(\'database\').innerHTML = "Unable to unzip the chat file. Error Code: ' . $zip . '";</script>';
			flush_headers();
			exit;
		}
		...
		// Bring chat out of maintenance mode
		echo '<script type="text/javascript">document.getElementById(\'maintenance_out\').innerHTML = "Bringing chat out of maintenance mode...";</script>';
		flush_headers();
		...
		// Delete the chat zip file
		echo '<script type="text/javascript">document.getElementById(\'unlink\').innerHTML = "Unlinking chat...";</script>';
		flush_headers();
		
		unlink(dirname(dirname(__FILE__)) . DIRECTORY_SEPARATOR . AC_FOLDER_CACHE . DIRECTORY_SEPARATOR . "chat_v" . get_var('url') . ".zip");
