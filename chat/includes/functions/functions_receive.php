<?php
	...
	*/
	function getAnnouncements() 
	{
		...
		{
			$result = $db->execute("
				SELECT chat_config.config_value announce, chat_status.announcement readstatus 
				FROM chat_status, chat_config 
				WHERE chat_status.userid='" . $db->escape_string($userid) . "'
					AND chat_config.config_name='announcement'
			");
	...
	*/
	function fetchMessages() 
	{
		...
		$result = $db->execute("
			SELECT chat.id, chat.from, chat.to, chat.message, chat.sent, chat.read, chat.user_read 
			FROM chat 
			WHERE chat.to = '" . $db->escape_string($userid) . "' 
				AND (chat.read != 1 
					OR chat.user_read != 1
					OR chat.sent > (" . $time_it . " - " . $db->escape_string($heart_beat) . " - 1))
		...
		if ($messages_test) 
		{
			...
			$db->execute("
				...
				SET chat.read = '1' 
				WHERE chat.to = '" . $db->escape_string($userid) . "' 
					AND chat.id <= '" . $db->escape_string($id_check) . "'
					AND chat.read = '0'
			");
