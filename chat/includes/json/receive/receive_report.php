	...
	{	
		...
		else
		{
			...
			$result = $db->execute("
				SELECT chat.id
				FROM chat
				WHERE ((chat.to = '" . $db->escape_string($report_about) . "' 
							AND chat.from = '" . $db->escape_string($report_from) . "') 
						OR (chat.from = '" . $db->escape_string($report_about) . "' 
							AND chat.to = '" . $db->escape_string($report_from) . "'))
					AND chat.sent <= " . $report_time . "
