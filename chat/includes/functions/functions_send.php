		...
		{ 
			if (!empty($smileys)) 
			{
				foreach ($smileys as $pattern => $result) 
				{
					...
					$text = str_replace($pattern, '<img class="chat_smiley" height="16" width="16" src="' . $base_url . AC_FOLDER_THEMES . '/' . $theme . '/images/smilies/' . $result . '.png" alt="' . $result . '">', $text);
				}
			}
		}
		
		$text = preg_replace('@video[{](.*)[}]@', '<div class="chat_action_message">' . $language[61] . '<br /><a href="javascript:jqac.chat.videoWith(\'$1\');">' . $language[62] . '</a></div>', $text);
		
		$text = preg_replace('@file[{]([0-9]{13})[}][{](.*)[}]@', '<div class="chat_action_message">' . $language[69] . '<br /><a href="' . $base_url . 'public/download.php?file=$1">$2</a></div>', $text);
		
		$text = preg_replace('@image[{]([0-9]{13})[}][{](.*)[}]@', '<div class="chat_image_message"><img data-id="' . $base_url . 'public/download.php?file=$1" src="' . $base_url . 'public/download.php?file=$1_t" alt="Image" class="chat_lightbox" /></div>', $text);
