<?php
	...
	$bookmarks_list			= get_var('chatapplist');
	...
	if (!empty($_POST['focus_chat'])) 
	{
		if ($tab_alert == "1") 
		{
			$db->execute("
				UPDATE chat 
				SET chat.user_read = '1', chat.read = '1'
				WHERE chat.from = '" . $db->escape_string($focus_chat) . "' 
	...
	if (!empty($_POST['unfocus_chat'])) 
	{
		if ($tab_alert == "1") 
		{
			$db->execute("
				UPDATE chat 
				SET chat.user_read = '1' , chat.read = '1'
				WHERE chat.from = '" . $db->escape_string($unfocus_chat) . "' 
					AND chat.to = '" . $db->escape_string($userid) . "' 
					AND chat.user_read = '0'
			");
		}
	...
	if (!empty($_POST['close_chat'])) 
	{
		if ($tab_alert == "1") 
		{
			$db->execute("
				UPDATE chat 
				SET chat.user_read = '1', chat.read = '1'
				WHERE chat.from = '" . $db->escape_string($close_chat) . "' 
					AND chat.to = '" . $db->escape_string($userid) . "' 
					AND chat.user_read = '0'
			");
		}
	...
	if (var_check('chatroom_mod')) 
	{
		...
		if ($result AND $db->count_select() > 0) 
		{
			...
			$db->execute("
				INSERT INTO chat_chatroom_messages (
					chatroom_id,
					user_id,
					username,
					message,
	...
	if (var_check('chatroom_ban')) 
	{
		...
		if ($result AND $db->count_select() > 0) 
		{
			...
			$db->execute("
				INSERT INTO chat_chatroom_banlist (
					user_id, 
					chatroom_id, 
					ban_length, 
					ban_time,
	...
	if (var_check('chat_applications')) 
	{
		$update_string = "";
	...
	if (var_check('chat_other_applications')) 
	{
		$update_string = "";
