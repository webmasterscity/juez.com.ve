<?php
	...
	*/
	function check_guest_hash()
	{
		...
		if (isset($_COOKIE['chat_guest_hash']))
		{
			$result = $db->execute("
				SELECT userid
				FROM chat_status
				WHERE hash_id = '".$db->escape_string($_COOKIE['chat_guest_hash'])."'
			");
	...
	*/
	function create_guest_hash()
	{
		...
		$result = $db->execute("
			INSERT INTO chat_status (userid, chatroom_window, chatroom_stay, hash_id, session_time) 
		...
		if ($result)
		{
			...
			setcookie('chat_guest_hash', $hash_id, time() + 3600 * 24 * 365 * 10, '/', $domain, false);
		}
	...
	*/
	function create_guest_username($userId, $guest_name = '', $get_username = false)
	{
		...
		{
			$result = $db->execute("
				SELECT DISTINCT chat_status.guest_name
				FROM chat_status
				WHERE chat_status.userid = '" . $db->escape_string($userId) . "'
			");
	...
	*/
	...
	function get_guest_details($userid)
	{
		...
		$sql = ("
			SELECT DISTINCT chat_status.userid, chat_status.session_time lastactivity, chat_status.status, chat_status.guest_name, chat_status.is_admin
			FROM chat_status
			WHERE chat_status.userid = '" . $db->escape_string($userid) . "'
		");
	...
	function get_guest_list() 
	{
		...
		$sql = ("
			...
			WHERE chat_status.session_time > (" . time() . " - " . $online_timeout . " - 60)
				AND LEFT(chat_status.userid, 1) = 'g'
