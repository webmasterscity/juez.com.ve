<?php
	...
	if (var_check('update')) 
	{	
		setcookie("chat_update_checked", "0", time() - 86400);
		...
		if ($result AND $db->count_select() > 0) 
		{
			...
			if (empty($error)) 
			{
				...
				{
					$error = "Your application is already up-to-date or you haven't uploaded the new application files to your server. Download the new files from the chat store and overwrite all the files on your server.";
				}
				
				if (empty($error)) 
				{
					$result = $db->execute("
						UPDATE chat_applications
	...
	if (var_check('add_chatroom_submit')) 
	{
		...
		{
			$error = "La selección de la llenura de la sala esta vacia o es invalida.";
		}
		...
		{		
			...
			$result = $db->execute("
				INSERT INTO chat_chatroom_rooms (
					author_id, 
					name, 
					description,
					welcome_message,
	...
	if (var_check('chatroom_edit_submit')) 
	{
		...
		{	
			...
			if ($usernames) 
			{
				...
				{
					$db->execute("
						DELETE FROM chat_chatroom_banlist 
	...
	if (var_check('f') AND $do == "appsettings") 
	{
		...
		$result = $db->execute("
			SELECT * 
			FROM chat_applications 
			WHERE chat_applications.folder = '" . $db->escape_string($folder_location) . "' 
				OR chat_applications.name = '" . $db->escape_string($application_name) . "'
		");
	...
	if (var_check('add_notification_submit')) 
	{
		...
		if (empty($error)) 
		{
			$result = $db->execute("
				SELECT MAX(type) 
				FROM chat_notifications_markup
			");
			$row = $db->fetch_array($result);
...
",  get_var('add_notification_markup'));
			...
			$result = $db->execute("
				INSERT INTO chat_notifications_markup (
					name, 
					type, 
					markup
				) 
			...
			if ($result) 
			{
				$msg = "Your notification was added successfully with type number " . $db->escape_string($type) . ".  You must add a mySQL statement where the action occurs before the notification will work.  Consult the chat documentation for more information.";
			} 
			else 
			{
				$error = "There was a database error.  Please try again.";
	...
	if (var_check('delete') AND $do == "notificationsettings") 
	{
		...
		{
			$result = $db->execute("
				DELETE FROM chat_notifications_markup 
				WHERE id = '" . $db->escape_string(get_var('delete')) . "'
			");
...
",  get_var('edit_notification_markup'));
			...
			$result = $db->execute("
				UPDATE chat_notifications_markup 
